...
export default function AddRegulation() {
  ...
  useEffect(() => {
    ...
    // Map category to prefix (first 2 letters of category, or first letter if only 1 word)
    const prefix = formData.category.length >= 2 
    ...
    // Format: [CategoryPrefix]-[Year]-[Random4Digits] (e.g., HR-2023-1234)
    setRefNumberPreview(`${prefix}-${year}-${randomNumber}`);
  ...
  const handleSaveAsDraft = async (e) => {
    ...
    try {
      ...
      console.log('Saving regulation as draft:', formDataToSubmit);
      
      // Add the regulation
      await addRegulation(formDataToSubmit);
  ...
  const handleSubmitForReview = async (e) => {
    ...
    try {
      ...
      console.log('Submitting regulation for review:', newRegulation);
        ...
        <form className="space-y-6">
          ...
          <div className="flex justify-end space-x-4 pt-6">
            ...
            <button
              ...
              disabled={isSaving}
              className="px-6 py-3 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700 transition disabled:opacity-50 disabled:cursor-not-allowed"
            >
