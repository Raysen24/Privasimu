...
export default function ReviewerDashboard() {
  ...
  const queue = useMemo(() => {
    // Things the reviewer can act on now
    return filtered
      .filter((r) => r.statusBucket === "needs_review" || r.statusBucket === "rejected")
      .sort((a, b) => {
  ...
  const deadlines = useMemo(() => {
    ...
    const rows = filtered
      ...
      .map((r) => ({ id: r.id, ref: r.ref, title: r.title, when: r.deadlineDate, bucket: r.statusBucket }));
